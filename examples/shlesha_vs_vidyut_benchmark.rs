...
fn benchmark_slp1(shlesha: &Slp1Converter, vidyut: &mut Lipika, text: &str) {
    ...
    let shlesha_throughput = (text.len() as f64 * iterations as f64) / shlesha_duration.as_secs_f64() / 1_000_000.0;
    ...
    println!("  📈 Shlesha:  {:.2} MB/s ({:?} avg, {} iterations)", shlesha_throughput, shlesha_duration / iterations, iterations);
    println!("  🚀 Vidyut:   {:.2} MB/s ({:?} avg, {} iterations)", vidyut_throughput, vidyut_duration / iterations, iterations);
    println!("  📊 Ratio:    Shlesha is {:.2}x {} than Vidyut", 
    ...
    println!("  ✅ Correctness: {} (Shlesha: {:?}, Vidyut: {:?})", 
             if format!("{:?}", shlesha_result).contains("dharma") && vidyut_result.contains("dharma") { "PASS" } else { "DIFF" },
...
fn benchmark_iast(shlesha: &IastConverter, vidyut: &mut Lipika, text: &str) {
    ...
    let shlesha_duration = start.elapsed();
    let shlesha_throughput = (iast_text.len() as f64 * iterations as f64) / shlesha_duration.as_secs_f64() / 1_000_000.0;
    ...
    let vidyut_duration = start.elapsed();
    let vidyut_throughput = (iast_text.len() as f64 * iterations as f64) / vidyut_duration.as_secs_f64() / 1_000_000.0;
