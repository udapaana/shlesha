...
fn main() {
    ...
    let scripts = vec![
        "iast", "itrans", "slp1", "harvard_kyoto", "kolkata", "velthuis", "wx",
        "iso15919_tokens", "devanagari_tokens", 
        "bengali", "tamil", "gujarati", "telugu"
    ];
        ...
        for to_script in &scripts {
            if from_script != to_script {
                match shlesha.transliterate(test_word, from_script, to_script) {
                    Ok(result) => {
                        println!("{} → {}: {} → {}", from_script, to_script, test_word, result);
                    }
                    Err(e) => {
