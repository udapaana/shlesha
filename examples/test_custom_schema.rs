...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match transliterator.transliterate("kam", "my_custom_encoding", "devanagari") {
        Ok(devanagari_result) => {
            println!("  my_custom_encoding -> devanagari: 'kam' -> '{}'", devanagari_result);
            
            // Try to convert back (this might not work perfectly yet)
            match transliterator.transliterate(&devanagari_result, "devanagari", "my_custom_encoding") {
                Ok(custom_result) => {
                    println!("  devanagari -> my_custom_encoding: '{}' -> '{}'", devanagari_result, custom_result);
                },
