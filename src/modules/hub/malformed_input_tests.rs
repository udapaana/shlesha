...
mod malformed_input_tests {
    ...
    fn test_whitespace_preservation() {
        ...
        assert!(extract_string(&deva_result).chars().all(|c| c.is_whitespace()));
        assert!(extract_string(&iso_result).chars().all(|c| c.is_whitespace()));
    }
    ...
    fn test_invalid_unicode_graceful_handling() {
        ...
        match result1 {
            Ok(output) => assert!(extract_string(&output).chars().all(|c| c.is_valid_unicode())),
    ...
    fn test_null_and_control_characters() {
        ...
        for input in control_inputs {
            ...
            match result1 {
                Ok(output) => {
                    assert!(extract_string(&output).chars().all(|c| c.is_valid_unicode()));
                },
