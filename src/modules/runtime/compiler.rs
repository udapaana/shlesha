...
impl RuntimeCompiler {
    ...
    pub fn compile_schema(&mut self, schema: &Schema) -> Result<CompiledProcessor, RuntimeCompilerError> {
        // Check cache first
        ...
        let generated_code = self.template_engine.render("token_based_converter", &template_data)?;
        ...
        self.cache_manager.store_cache(&cache_key, &compilation_cache)?;
    ...
    fn prepare_template_data(&self, schema: &Schema) -> Result<Value, RuntimeCompilerError> {
        ...
        // Basic metadata
        template_data.insert("script_name".to_string(), Value::String(schema.metadata.name.clone()));
        template_data.insert("struct_name".to_string(), Value::String(format!("{}Converter", schema.metadata.name)));
            ...
            for (token, mapping) in entries {
                ...
                match mapping {
                    Value::String(single_input) => {
                        ...
                        entry.insert("all_inputs".to_string(), Value::Array(vec![Value::String(single_input.clone())]));
                    }
                    ...
                    _ => return Err(RuntimeCompilerError::CompilationError("Invalid mapping format".to_string())),
                }
        ...
        let has_multi_char = schema.mappings.values()
        ...
        template_data.insert("has_multi_char_mappings".to_string(), Value::Bool(has_multi_char));
        
        Ok(Value::Object(template_data))
    }

    fn create_temp_crate(&mut self, schema: &Schema, generated_code: &str) -> Result<PathBuf, RuntimeCompilerError> {
