...
use super::generic_phf::{GenericCharMap, get_telugu_to_deva_map, get_deva_to_telugu_map};

/// Telugu script converter with generic PHF (Perfect Hash Function) optimization
/// 
...
/// 
/// OPTIMIZATION: Uses generic PHF optimization that automatically chooses between
/// compile-time perfect hash tables, FxHashMap, or HashMap based on size.
pub struct TeluguConverter {
    // Use generic optimized character maps
    char_maps: GenericCharMap,
}
...
impl TeluguConverter {
    pub fn new() -> Self {
        ...
        // Use pre-generated PHF maps for optimal performance
        let char_maps = GenericCharMap::with_phf_maps(
            get_telugu_to_deva_map(),
            get_deva_to_telugu_map()
        );
    ...
    pub fn devanagari_to_telugu(&self, input: &str) -> Result<String, ConverterError> {
        let mut result = String::new();
        let phf_map = get_deva_to_telugu_phf();
        
        // Simple character-to-character mapping
        for ch in input.chars() {
            if ch.is_whitespace() || ch.is_ascii_punctuation() {
                result.push(ch);
            } else if let Some(&telugu_char) = phf_map.get(&ch) {
