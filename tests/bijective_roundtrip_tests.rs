...
fn test_bijective_roundtrip_ll_character() {
    ...
    let bengali_result = shlesha.transliterate(original, "devanagari", "bengali")
        .expect("Failed to convert to Bengali");
    
    println!("Original: {} -> Bengali: {}", original, bengali_result);
    
    let roundtrip_result = shlesha.transliterate(&bengali_result, "bengali", "devanagari")
        .expect("Failed to convert back to Devanagari");
    
    println!("Bengali: {} -> Roundtrip: {}", bengali_result, roundtrip_result);
    
    assert_eq!(original, roundtrip_result, 
...
fn test_bijective_roundtrip_multiple_scripts() {
    ...
    for (original_char, source_script) in test_cases {
        for target_script in &target_scripts {
            ...
            let converted = shlesha.transliterate(original_char, source_script, target_script)
                .expect(&format!("Failed to convert {} from {} to {}", 
                    original_char, source_script, target_script));
            
            // Convert back to source script
            let roundtrip = shlesha.transliterate(&converted, target_script, source_script)
                .expect(&format!("Failed to convert back from {} to {}", 
                    target_script, source_script));
            
            assert_eq!(original_char, roundtrip,
                "Bijective roundtrip failed: {} -> {} -> {} -> {}. Expected: {}, Got: {}",
                original_char, source_script, target_script, source_script, original_char, roundtrip);
        }
    }
}
...
fn test_bijective_roundtrip_with_common_characters() {
    ...
    let bengali_result = shlesha.transliterate(common_text, "devanagari", "bengali")
        .expect("Failed to convert to Bengali");
    
    let roundtrip_result = shlesha.transliterate(&bengali_result, "bengali", "devanagari")
        .expect("Failed to convert back to Devanagari");
    
    assert_eq!(common_text, roundtrip_result,
        "Bijective roundtrip failed for common text: {}", common_text);
}
...
fn test_runtime_token_allocation() {
    ...
    assert!(result.is_ok(), "Basic transliteration should work with runtime token architecture");
}
...
fn test_preservation_priority_devanagari() {
    ...
    assert_eq!(original.as_bytes(), roundtrip_result.as_bytes(),
        "Should preserve Devanagari ळ (bytes: {:?}), not Gujarati ળ. Got bytes: {:?}",
        original.as_bytes(), roundtrip_result.as_bytes());
}