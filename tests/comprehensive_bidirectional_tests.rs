...
fn test_all_script_pairs() {
    ...
    for (from_script, from_text, _) in &test_cases {
        for (to_script, _, _) in &test_cases {
            ...
            match result {
                Ok(output) => {
                    println!("✓ {} → {}: '{}' → '{}'", from_script, to_script, from_text, output);
                    passed += 1;
                },
                Err(e) => {
                    println!("✗ {} → {}: '{}' → ERROR: {}", from_script, to_script, from_text, e);
                    failed += 1;
                }
            }
        }
    ...
    assert!(passed > 40, "Expected at least 40 working conversions, got {}", passed);
}
...
fn test_indic_to_roman_conversions() {
    ...
    for (indic_script, text) in &indic_scripts {
        for roman_script in &roman_scripts {
            ...
            match result {
                Ok(output) => println!("✓ {} → {}: '{}' → '{}'", indic_script, roman_script, text, output),
                Err(e) => println!("✗ {} → {}: '{}' → ERROR: {}", indic_script, roman_script, text, e),
            }
        }
    }
}
...
fn test_roman_to_indic_conversions() {
    ...
    for (roman_script, text) in &roman_texts {
        for indic_script in &indic_scripts {
            ...
            match result {
                Ok(output) => println!("✓ {} → {}: '{}' → '{}'", roman_script, indic_script, text, output),
                Err(e) => println!("✗ {} → {}: '{}' → ERROR: {}", roman_script, indic_script, text, e),
            }
        }
    }
}
...
fn test_roundtrip_conversions() {
    ...
    for (script, original_text) in test_cases {
        ...
        match t.transliterate(original_text, script, "iso") {
            Ok(iso_text) => {
                match t.transliterate(&iso_text, "iso", script) {
                    Ok(roundtrip_text) => {
                        if roundtrip_text == original_text {
                            println!("✓ {} roundtrip through ISO: '{}' → '{}' → '{}' (exact match)", 
                                     script, original_text, iso_text, roundtrip_text);
                        } else {
                            println!("⚠ {} roundtrip through ISO: '{}' → '{}' → '{}' (different)", 
                                     script, original_text, iso_text, roundtrip_text);
                        }
                    },
