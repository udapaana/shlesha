...
fn get_all_supported_scripts() -> Vec<String> {
    ...
    scripts.into_iter()
        .filter(|script| {
            ...
            matches!(script.as_str(), 
                "devanagari" | "bengali" | "gujarati" | "telugu" | "tamil" | 
                "kannada" | "malayalam" | "odia" | "gurmukhi" | 
...
fn categorize_scripts() -> (Vec<String>, Vec<String>) {
    ...
    let indic_scripts: Vec<String> = all_scripts.iter()
        .filter(|script| {
            matches!(script.as_str(),
                "devanagari" | "bengali" | "gujarati" | "telugu" | "tamil" | 
                "kannada" | "malayalam" | "odia" | "gurmukhi"
    ...
    let roman_scripts: Vec<String> = all_scripts.iter()
        .filter(|script| {
            matches!(script.as_str(),
                "iast" | "iso15919" | "harvard_kyoto" | "slp1" | "itrans" | "velthuis"
            )
...
fn test_comprehensive_roundtrip_all_scripts() {
    ...
    println!("Testing {} Indic scripts and {} Roman scripts", 
             indic_scripts.len(), roman_scripts.len());
    
    // Test the comprehensive string across all scripts
    for test_string in [COMPREHENSIVE_TEST_STRING].iter().chain(EDGE_CASE_STRINGS.iter()) {
...
fn test_single_roundtrip(
    ...
    let roundtrip = match shlesha.transliterate(&converted, target_script, source_script) {
        Ok(result) => result,
        Err(e) => {
            println!("  ❌ {} → {} → {}: Forward succeeded but reverse failed: {}", 
...
fn prop_devanagari_roundtrip(text: String) -> TestResult {
    ...
    let devanagari_text: String = text.chars()
        .filter(|c| {
            let code = *c as u32;
    ...
    for target_script in ["bengali", "gujarati", "telugu"].iter() {
        if indic_scripts.contains(&target_script.to_string()) {
            let converted = match shlesha.transliterate(&devanagari_text, "devanagari", target_script) {
                Ok(result) => result,
...
fn test_all_scripts_handle_comprehensive_string() {
    ...
    for source_script in &all_scripts {
        for target_script in &all_scripts {
            ...
            let test_text = if source_script == "devanagari" {
                ...
                match shlesha.transliterate(COMPREHENSIVE_TEST_STRING.trim(), "devanagari", source_script) {
                    Ok(text) => text,
                    Err(_) => continue,
                }
            };
            ...
            match shlesha.transliterate(&test_text, source_script, target_script) {
                Ok(result) => {
                    ...
                    // Basic sanity checks
                    assert!(!result.is_empty(), "Empty result for {} → {}", source_script, target_script);
                    
                    // Result should not be just question marks (indicates mapping failure)
                    let question_mark_ratio = result.chars().filter(|&c| c == '?').count() as f64 / result.chars().count() as f64;
                    assert!(question_mark_ratio < 0.5, 
                           "Too many unmapped characters ({:.1}%) for {} → {}: '{}'", 
                           question_mark_ratio * 100.0, source_script, target_script, 
    ...
    println!("Script conversion success rate: {}/{} ({:.1}%)", 
             successful_conversions, total_attempts, success_rate * 100.0);
