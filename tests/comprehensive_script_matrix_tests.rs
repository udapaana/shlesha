...
const ALL_SCRIPTS: &[&str] = &[
    // Roman scripts
    "iast", "slp1", "harvard_kyoto", "itrans", "velthuis", "wx", "kolkata",
    // Indic scripts  
...
fn test_roman_to_roman_conversions() {
    ...
    let available_roman: Vec<&str> = ROMAN_SCRIPTS.iter()
    ...
    for &source in &available_roman {
        for &target in &available_roman {
            ...
            let source_test_cases: Vec<_> = get_test_cases().into_iter()
        ...
        for &target in &available_roman {
            if source != target {
                if let Some((succeeded, total)) = conversion_matrix.get(&(source, target)) {
                    let percentage = if *total > 0 { succeeded * 100 / total } else { 0 };
                    println!("{:<15} {}/{} ({}%)", 
                        format!("{}→{}", source, target), succeeded, total, percentage);
                }
            }
        }
    }
...
fn test_indic_to_indic_conversions() {
    ...
    let available_indic: Vec<&str> = INDIC_SCRIPTS.iter()
        .filter(|script| supported_scripts.contains(&script.to_string()))
        .copied()
        .collect();
    ...
    for &source in &available_indic {
        for &target in &available_indic {
            ...
            let devanagari_test_cases: Vec<_> = get_test_cases().into_iter()
...
fn test_roman_to_indic_conversions() {
    ...
    for &roman in &available_roman {
        for &indic in &available_indic {
            ...
            let test_cases: Vec<_> = get_test_cases().into_iter()
    ...
    for &roman in &available_roman {
        for &indic in &available_indic {
            if let Some((succeeded, total)) = conversion_matrix.get(&(roman, indic)) {
                let percentage = if *total > 0 { succeeded * 100 / total } else { 0 };
                println!("{:<20} {}/{} ({}%)", 
                    format!("{}→{}", roman, indic), succeeded, total, percentage);
            }
        }
    }
    ...
    for &indic in &available_indic {
        for &roman in &available_roman {
            if let Some((succeeded, total)) = conversion_matrix.get(&(indic, roman)) {
                let percentage = if *total > 0 { succeeded * 100 / total } else { 0 };
                println!("{:<20} {}/{} ({}%)", 
                    format!("{}→{}", indic, roman), succeeded, total, percentage);
            }
        }
    }
...
fn test_round_trip_consistency() {
    ...
    for &script1 in &available_roman {
        for &script2 in &available_roman {
            ...
            for test_case in &test_cases {
                ...
                match shlesha.transliterate(test_case.text, script1, script2)
    ...
    println!("\n🔄 Round-trip Results:");
    println!("Successful round-trips: {}/{} ({}%)", 
        round_trip_successes, round_trips_tested, 
        if round_trips_tested > 0 { round_trip_successes * 100 / round_trips_tested } else { 0 });
...
fn test_comprehensive_coverage_report() {
    ...
    println!("✓ Identity conversions: {} scripts", supported_scripts.len());
