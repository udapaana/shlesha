...
fn test_dynamic_script_discovery() {
    ...
    println!("🔍 Discovered {} scripts: {:?}", all_scripts.len(), all_scripts);
    
    // Ensure we found a reasonable number of scripts
    assert!(all_scripts.len() >= 10, "Expected at least 10 scripts, found {}", all_scripts.len());
...
fn categorize_scripts(shlesha: &Shlesha) -> (Vec<String>, Vec<String>) {
    ...
    let known_indic_patterns = [
        ...
        "odia", "tamil", "telugu", "grantha", "sinhala", "tibetan"
    ];
    
    for script in all_scripts {
        let script_lower = script.to_lowercase();
        
        if known_roman_patterns.iter().any(|pattern| script_lower.contains(pattern)) {
            roman_scripts.push(script);
        } else if known_indic_patterns.iter().any(|pattern| script_lower.contains(pattern)) {
    ...
    println!("📜 Roman scripts ({}): {:?}", roman_scripts.len(), roman_scripts);
    println!("🔤 Indic scripts ({}): {:?}", indic_scripts.len(), indic_scripts);
...
fn generate_test_data() -> HashMap<String, Vec<&'static str>> {
    ...
    test_data.insert("iast".to_string(), vec![
        "a", "ā", "i", "ī", "u", "ū", "ṛ", "ṝ", "e", "o", "ai", "au",
        "ka", "kha", "ga", "gha", "ṅa", "ca", "cha", "ja", "jha", "ña",
        "ṭa", "ṭha", "ḍa", "ḍha", "ṇa", "ta", "tha", "da", "dha", "na",
        "pa", "pha", "ba", "bha", "ma", "ya", "ra", "la", "va",
        "śa", "ṣa", "sa", "ha", "ṃ", "ḥ", "kṣa", "jña",
        "saṃskṛtam", "dharmakṣetre", "namaskāram"
    ]);
    
    // SLP1-specific test cases
    test_data.insert("slp1".to_string(), vec![
        "a", "A", "i", "I", "u", "U", "f", "F", "x", "X", "e", "o", "E", "O",
        "ka", "Ka", "ga", "Ga", "Na", "ca", "Ca", "ja", "Ja", "Ya",
        "wa", "Wa", "qa", "Qa", "Ra", "ta", "Ta", "da", "Da", "na",
        "pa", "Pa", "ba", "Ba", "ma", "ya", "ra", "la", "va",
        "Sa", "za", "sa", "ha", "M", "H", "kza", "jYa",
        "saMskftam", "Darmakzetre", "namaskAram"
    ]);
    
    // Devanagari test cases
    test_data.insert("devanagari".to_string(), vec![
        "अ", "आ", "इ", "ई", "उ", "ऊ", "ऋ", "ॠ", "ए", "ओ", "ऐ", "औ",
        "क", "ख", "ग", "घ", "ङ", "च", "छ", "ज", "झ", "ञ",
        "ट", "ठ", "ड", "ढ", "ण", "त", "थ", "द", "ध", "न",
        "प", "फ", "ब", "भ", "म", "य", "र", "ल", "व",
        "श", "ष", "स", "ह", "ं", "ः", "क्ष", "ज्ञ",
        "संस्कृतम्", "धर्मक्षेत्रे", "नमस्कारम्"
    ]);
    
    // For other scripts, use universal test cases
    for script in ["harvard_kyoto", "itrans", "velthuis", "wx", "kolkata", 
                   "bengali", "gujarati", "gurmukhi", "kannada", "malayalam",
                   "odia", "tamil", "telugu", "grantha", "sinhala"] {
...
fn test_all_identity_conversions() {
    ...
    for script in &all_scripts {
        ...
        let test_cases = test_data.get(script)
            .unwrap_or(&default_cases); // Default cases
        
        for &test_input in test_cases.iter().take(5) { // Limit for performance
            total_tests += 1;
    ...
    println!("   Success rate: {:.1}%", 
        if total_tests > 0 { (total_tests - failed_tests) as f64 / total_tests as f64 * 100.0 } else { 0.0 });
...
fn test_all_script_pairs_matrix() {
    ...
    for source_script in &all_scripts {
        for target_script in &all_scripts {
            ...
            let test_cases = test_data.get(source_script)
    ...
    println!("   Success rate: {:.1}%", 
        if total_pairs > 0 { working_pairs as f64 / total_pairs as f64 * 100.0 } else { 0.0 });
    ...
    println!("\n💾 Conversion matrix stored for analysis ({} pairs)", conversion_results.len());
}
...
fn test_character_mapping_consistency() {
    ...
    if mapping_failures.len() > total_mapping_tests / 2 {
        panic!("Too many character mapping failures: {}/{}", 
            mapping_failures.len(), total_mapping_tests);
    }
}
...
fn test_round_trip_all_scripts() {
    ...
    println!("   Success rate: {:.1}%", 
        if total_round_trips > 0 { successful_round_trips as f64 / total_round_trips as f64 * 100.0 } else { 0.0 });
    ...
    if failure_rate > 0.5 {
        panic!("Excessive round-trip failures: {:.1}% failure rate", failure_rate * 100.0);
    }
}
...
fn test_performance_all_scripts() {
    ...
    let slow_scripts: Vec<_> = sorted_results.iter()
            ...
            Ok(result) => {
                if !result.is_empty() {
                    property_violations.push(format!(
                        "Empty input property violated: {} '' → '{}'", script, result
                    ));
                }
            }
            Err(_) => {
...
fn test_system_health_summary() {
    ...
    println!("   Total possible conversions: {}", all_scripts.len() * (all_scripts.len() - 1));
