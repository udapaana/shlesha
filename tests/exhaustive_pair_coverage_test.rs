...
impl CharsetTestResults {
    ...
    fn total_tests(&self) -> usize {
        self.successful_chars.len() + self.failed_chars.len() + 
        self.successful_combinations.len() + self.failed_combinations.len()
    }
...
fn generate_script_character_sets() -> HashMap<String, Vec<String>> {
    ...
    // IAST character set
    character_sets.insert("iast".to_string(), vec![
        // Vowels
    ...
    // SLP1 character set
    character_sets.insert("slp1".to_string(), vec![
        // Vowels
    ...
    character_sets.insert("harvard_kyoto".to_string(), vec![
        // Vowels
    ...
    character_sets.insert("devanagari".to_string(), vec![
        // Vowels
    ...
    let default_roman_charset: Vec<String> = vec![
        "a", "i", "u", "e", "o", "k", "g", "c", "j", "t", "d", "p", "b", "m", "n", "r", "l", "v", "s", "h"
    ].into_iter().map(|s| s.to_string()).collect();
    ...
    for script in ["bengali", "gujarati", "gurmukhi", "kannada", "malayalam", "odia", "tamil", "telugu", "grantha", "sinhala"] {
...
fn get_charset_for_script(script: &str, character_sets: &HashMap<String, Vec<String>>) -> Vec<String> {
    ...
    if is_roman_script(script) {
        character_sets.get("iast").unwrap_or(&vec!["a".to_string()]).clone()
    } else {
        character_sets.get("devanagari").unwrap_or(&vec!["क".to_string()]).clone()
    }
}
...
fn analyze_charset_results_by_script_type(
    ...
    fn print_stats(category: &str, stats: &[f64]) {
        ...
        println!("   {}: {} pairs, avg {:.1}%, range {:.1}%-{:.1}%", 
            category, stats.len(), avg, min, max);
    }
...
fn generate_character_coverage_matrix(
    ...
    for pair_results in results.values() {
        ...
        total_combinations_tested += pair_results.successful_combinations.len() + pair_results.failed_combinations.len();
        total_combinations_successful += pair_results.successful_combinations.len();
    }
    
    println!("   Single characters: {}/{} successful ({:.1}%)", 
        total_chars_successful, total_chars_tested,
        if total_chars_tested > 0 { total_chars_successful as f64 / total_chars_tested as f64 * 100.0 } else { 0.0 });
    
    println!("   Character combinations: {}/{} successful ({:.1}%)", 
        total_combinations_successful, total_combinations_tested,
        if total_combinations_tested > 0 { total_combinations_successful as f64 / total_combinations_tested as f64 * 100.0 } else { 0.0 });
    ...
    for pair_results in results.values() {
        for failed_char in pair_results.failed_chars.keys() {
            *character_failure_counts.entry(failed_char.clone()).or_insert(0) += 1;
        }
    }
    ...
    if !character_failure_counts.is_empty() {
        ...
        for (character, failure_count) in sorted_failures.into_iter().take(10) {
            println!("   '{}': failed in {} conversions", character, failure_count);
        }
    }
}
...
fn test_exhaustive_script_pair_coverage() {
    ...
    println!("Expected total pairs: {} × {} = {}", total_scripts, total_scripts - 1, expected_pairs);
    ...
    println!("Generated {} unique pairs for testing", all_possible_pairs.len());
    assert_eq!(all_possible_pairs.len(), expected_pairs, 
    ...
    for (source, target) in &all_possible_pairs {
        pair_count += 1;
        if pair_count % 100 == 0 {
            println!("   Progress: {}/{} pairs tested", pair_count, expected_pairs);
        }
    ...
    assert_eq!(tested_pairs.len(), all_possible_pairs.len(), 
    ...
    println!("   Total coverage: {}/{} pairs (100%)", tested_pairs.len(), all_possible_pairs.len());
}
...
fn is_roman_script(script: &str) -> bool {
    ...
    let script_lower = script.to_lowercase();
    roman_patterns.iter().any(|pattern| script_lower.contains(pattern))
}
...
fn test_script_pairs_with_complete_charset() {
    ...
    println!("Testing {} script pairs with complete character sets", total_pairs);
    ...
    for (source, target) in all_scripts.iter()
        ...
        for character in &charset {
            ...
            match shlesha.transliterate(character, source, target) {
                Ok(result) => {
                    ...
                    pair_results.successful_chars.insert(character.clone(), result);
                }
                Err(e) => {
                    failed_char_tests += 1;
                    pair_results.failed_chars.insert(character.clone(), e.to_string());
                }
            }
        }
        ...
        for i in 0..charset.len().min(10) {
            for j in (i+1)..charset.len().min(10) {
                ...
                match shlesha.transliterate(&combination, source, target) {
                    Ok(result) => {
                        successful_char_tests += 1;
                        pair_results.successful_combinations.insert(combination, result);
                    }
                    Err(e) => {
                        failed_char_tests += 1;
                        pair_results.failed_combinations.insert(combination, e.to_string());
                    }
                }
            }
        }
    ...
    println!("   Character success rate: {:.1}%", 
    ...
    println!("   Total coverage: {} character tests across {} pairs", total_char_tests, total_pairs);
}
...
fn test_charset_round_trip_consistency() {
    ...
    let roman_scripts: Vec<_> = all_scripts.iter()
        .filter(|script| is_roman_script(script))
        .collect();
    
    println!("Testing round-trips between {} Roman scripts", roman_scripts.len());
    
    for script1 in &roman_scripts {
        for script2 in &roman_scripts {
            if script1 == script2 { continue; }
            
            let charset = get_charset_for_script(script1, &character_sets);
            
            // Test round-trip for each character in the charset
            for character in charset.iter().take(20) { // Limit for performance
                total_round_trip_tests += 1;
                
                // script1 → script2 → script1
                match shlesha.transliterate(character, script1, script2)
            ...
            for i in 0..limited_charset.len() {
                for j in (i+1)..limited_charset.len() {
                    ...
                    match shlesha.transliterate(&combination, script1, script2)
                        .and_then(|intermediate| 
                            shlesha.transliterate(&intermediate, script2, script1)) {
                        Ok(final_result) => {
...
fn test_generate_complete_coverage_matrix() {
    ...
    println!("Successful pairs: {} ({:.1}%)", successful_pairs, 
        successful_pairs as f64 / total_pairs as f64 * 100.0);
    println!("Failed pairs: {} ({:.1}%)", failed_pairs,
    ...
    for source in &sample_scripts {
        ...
        for target in &sample_scripts {
            if source == target {
                ...
                let result = matrix.get(&((*source).clone(), (*target).clone()))
    ...
    assert_eq!(matrix.len(), all_scripts.len() * (all_scripts.len() - 1),
