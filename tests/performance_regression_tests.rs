...
fn test_basic_conversion_performance() {
    ...
    let (result, duration) = measure_time(|| {
        transliterator.transliterate(SMALL_TEXT, "devanagari", "iso15919")
    });
...
fn test_reverse_conversion_performance() {
    ...
    let (result, duration) = measure_time(|| {
        transliterator.transliterate("dharma", "iso15919", "devanagari")
    });
...
fn test_medium_text_performance() {
    ...
    let (result, duration) = measure_time(|| {
        transliterator.transliterate(MEDIUM_TEXT, "devanagari", "iast")
    });
...
fn test_large_text_performance() {
    ...
    let (result, duration) = measure_time(|| {
        transliterator.transliterate(LARGE_TEXT, "devanagari", "itrans")
    });
...
fn test_roman_to_roman_performance() {
    ...
    let (result, duration) = measure_time(|| {
        transliterator.transliterate("namaste karma dharma", "iast", "itrans")
    });
...
fn test_multiple_conversions_performance() {
    ...
    assert!(results.iter().all(|r| r.is_ok()), "All conversions should succeed");
...
fn test_transliterator_creation_performance() {
    ...
    let (transliterator, duration) = measure_time(|| {
        Shlesha::new()
    });
...
fn test_repeated_text_performance() {
    ...
    let (result, duration) = measure_time(|| {
        transliterator.transliterate(&repeated_text, "devanagari", "iast")
    });
...
fn test_concurrent_performance() {
    ...
    assert!(results.iter().all(|r| r.is_ok()), "All sequential conversions should succeed");
...
fn test_schema_loading_performance() {
    ...
    let (mut transliterator, creation_duration) = measure_time(|| {
        Shlesha::new()
    });
...
fn test_error_path_performance() {
    ...
    let (result, duration) = measure_time(|| {
        transliterator.transliterate("test", "nonexistent_script", "devanagari")
    });
